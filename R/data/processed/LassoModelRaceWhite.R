...
data_race_hispanic_only = data_core %>% select(-matches("black|white|asian|aian|rawvalue|other"),-county) %>% 
...
set.seed(1)
split = initial_split(data_race_hispanic_only, prop=0.7, strata="v005_race_hispanic", breaks=5)
...
target_name = "v005_race_hispanic"
recipe_sat = 
  recipe(v005_race_hispanic~ ., data=train_data) %>%
