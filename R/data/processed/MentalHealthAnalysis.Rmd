...
Collect vars with race breakdown
```{r}
...
PHS vs Poor Mental health days raw
```{r}
...
desc <- readRDS("../../../data/processed/colDesc_analysis2023.rds")
for( var in v005_race_vars){
v145byraceVsPMHD <- ggplot(data_analytic, aes_string(y = paste0("log(",var,")"), x = "v145_rawvalue"))+
  geom_point(color="green", alpha =0.5)+
  geom_smooth()+
  xlab(desc[["v145_rawvalue"]]) +
  ylab(desc[[var]])+
...
for(var in v138_race_vars){
  va138byracegraph <- ggplot(data_analytic, aes_string(y = "(v005_rawvalue)", x = paste0("log(",var,")")))+
    geom_point(color="green", alpha =0.5)+
    geom_smooth()+
  xlab(desc[[var]]) +
  ylab(desc[["v005_rawvalue"]])+
