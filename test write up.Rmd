...
data_bi1 = bi_class(bi_chloro_plot, x = v005_rawvalue, y = v004_other_data_1, style = "quantile", dim = 3)
...
#county_bivar = county_bivar %>% select(-long) %>% 
#select(-lat)

#county_bivar %>% relocate("geometry", .after = "county")
...
map <- ggplot() +
  geom_sf(data = county_bivar, mapping = aes(fill = biVar, geometry = geometry), color = "black", size = 0.1, show.legend = FALSE) +
