...
for (subgroup in races) {
  plot <- ggplot(access_care, aes(x = log(v004_other_data_1), y = log(get(paste0("v005_race_", subgroup))))) +
    ...
    labs(y = paste("PHS for", subgroup)) + xlim(5, 10) +ylim(5,11) + geom_smooth() +geom_smooth(method = "lm", color = "red")
  print(plot)
}
...
for (subgroup_mental in races) {
  plot <- ggplot(access_care, aes(x = log(v062_other_data_1), y = log(get(paste0("v005_race_", subgroup_mental))))) +
    ...
    labs(y = paste("PHS for", subgroup_mental)) +xlim(2.5, 11) +ylim(5,12)+ geom_smooth() + annotate(11,12, label=(paste0("slope==",coef(lm(access_care$(get(paste0("v005_race_", subgroup_mental)))~access_care$v062_other_data_1)))))
    print(plot)
}
